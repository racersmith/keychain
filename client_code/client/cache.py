...
def get_match_from_path(path: str) -> Route:
    ...
    location = get_nav_location(
        generic_path, path=None, query=None, params=None, hash=None
    )
...
def invalidate(
    ...
    """
    fields_or_keys = utils.ensure_set(field_or_key)

    for path in utils.ensure_set(path):
...
def _set(field: str, value, missing_value, **loader_args):
    ...
    if value is not missing_value and field in _FIELDS:
        _DATA[key] = value
        if key != field:
